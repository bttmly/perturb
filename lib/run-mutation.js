...
function finish (done, domain) {
  return function (mutant, reporter) {
    var shouldReport = global.__perturb__.verbose || !util.mutantIsDead(mutant);
...
function runMutationWithIo (mutation, mutantReporter, done) {
  ...
  var runnerName = global.__perturb__.runner || "mocha";
  ...
  d.on("error", function (err) {
    ...
    process.nextTick(function () {
      ...
      finish(formatMutation(mutation), mutantReporter);
    });
  });

  d.run(runner(mutation, mutantReporter, finish(done, d)));
}

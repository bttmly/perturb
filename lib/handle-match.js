...
function handleMatch (match, done) {
  ...
  async.parallel({
    ...
    var mutations = mutationPaths.map(partial(mutationFromPath, immutableAst, files));

    mutations.forEach(function (m) {
      console.log(m.mutation, m.sourceFile);
    });
...
function mutationFromPath (iAst, files, path) {

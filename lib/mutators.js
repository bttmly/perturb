...
mutators = Object.freeze({
  ...
  swapLogicalOperators: function swapLogicalOperators (node) {
    assertNodeTypeIs(node, LOGICAL_EXPRESSION); {
      var prevOp = node.get("operator");
      var newOp = prevOp === "&&" ? "||" : "&&";
      return node.set("operator", newOp);
    }
  ...
  swapBinaryOperators: function swapBinaryOperators (node) {
    assertNodeTypeIs(node, BINARY_EXPRESSION);
    var prevOp = node.get("operator");
  ...
  // `return +maybeNum` => `return maybeNum`
  dropUnaryOperator: function dropUnaryOperator (node) {
    assertNodeTypeIs(node, UNARY_EXPRESSION);
...
nodeTypeToMutatorMap = {
  ...
  LogicalExpression: mutators.swapLogicalOperators,

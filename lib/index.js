...
module.exports = function (settings, cb) {
  ...
  exec(cmd, function (err, out) {
    if (err)
      return cb(new util.ChildProcessError(ERRORS.TestsFailed, cmd, out, err.code));
...
function perturb (settings, cb) {
  ...
  // and ending here ...
  // is all filesystem IO, move it into a function which just takes the config object

  var matches = perturbMatch(perturbSourceFiles, perturbTestFiles, config.matcher.bind(config));

  if (matches.length === 0) {
    cb(new Error(ERRORS.NoMatches));
  }

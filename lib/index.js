...
module.exports = function (settings, cb) {
  ...
  exec(cmd, function (err, out) {
    ...
    console.log("tests passed, perturbing...");
    perturb(settings, cb);
  });
};
...
function perturb (settings, cb) {
  ...
  async.mapSeries(matches, handleMatch, function (err, matches) {
    ...
    meta.duration = util.hrtimeToSeconds(process.hrtime(start));
    meta.errored = !!err;
...
function defaultConfig () {
  ...
  var config = {
    ...
    // '/example/x/y/z/thing.js' will match '/test/x/y/z/thing.js'
    matcher: function defaultMatcher (sourceFile, testFile) {
      var sName = sourceFile.split(path.join(PERTURB_DIR, this.sourceDir)).pop();
      var tName = testFile.split(path.join(PERTURB_DIR, this.testDir)).pop();
      return sName === tName;
    },
  };

...
function createComparativeMatcher (config) {
  return function comparativeMatcher (sourceFile, testFile) {
    var sourceName = sourceFile.split(path.join(config.perturbDir, config.sourceDir)).pop();
    var testName = testFile.split(path.join(config.perturbDir, config.testDir)).pop();
